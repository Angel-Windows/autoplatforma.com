...
$base_name = "";
switch ($item_card->base_name) {
    case 'lots_item_copart':
        $uppercase = "uppercase";
        $base_name = 'copart';
        break;
    case 'lots_item_iaai':
...
<article
    ...
    <div class="relative">
        <a href="{{route("page.auto_pop",['car_id'=>$item_card->id])}}">
            <img class="rounded-t-[15px] w-full" src="{{asset($item_card->attachments_url ?? "img/card/01.png")}}"
                 alt="auto">
        </a>
        <!-- primary block -->
        <div
    ...
    <div class="p-5 pb-2.5">
        ...
        <p class="leading-tight text-xs font-medium text-gray-500 mb-1 ">
            Condition: {{$item_card->lot_runs_drive_name}}</p>
        <p class="leading-tight text-xs font-medium text-gray-500 mb-1 ">Primary Damage:
            {{$item_card->lot_damage_name}}</p>
    ...
    <div class=" py-2 p-5 flex justify-between">
        ...
        <div class="flex gap-1">
            ...
            <p class="text-xs font-medium text-gray-900 dark:text-gray-300">Fuel: {{$item_card->fuel_name}}</p>
        </div>
    </div>
</article>
